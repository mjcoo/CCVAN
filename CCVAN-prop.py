...
df = pd.read_csv(r'D:\data\proteins_data\cyp2d6\datasets\cyp2d6.csv')
...
score = df['DOCKING_SCORE'].tolist()
smiles_tensor = torch.tensor(one_hot_list, dtype=torch.float32)
cond1_tensor = torch.tensor(score)
...
dataset = TensorDataset(X_train_tensor,cond1_tensor)
...
class CVAE(nn.Module):
    def __init__(self, n_input=N_INPUT,embedding_dim=128, n_hidden1=512, n_hidden2=256, n_z=100):
        ...
        self.fc1 = nn.Linear(53248+1, n_hidden1)
        self.fc2 = nn.Linear(n_hidden1, n_z)
        self.fc3 = nn.Linear(n_hidden1, n_z)
        self.fc4 = nn.Linear(n_z+1, n_hidden1)
    ...
    def encode(self, x,c):
        ...
        h = torch.cat([attn_applied.view(attn_applied.size(0), -1), c], dim=1)
...
def generate_smiles(cvae,logp,batch) :
    cvae.eval()
    gen_smi=[]
    z = torch.randn(batch, latent_dim*2)
    c1 = torch.zeros(batch,1)
    c1[:] = logp
    with torch.no_grad():
        output = cvae.decode(z, c1)
        smiles = get_best_smile(output,c1)

    return smiles

def draw_molecules(cvae, logps) :
    for logp in logps:
        #for mr in mrs:
            generated_smiles = generate_smiles(cvae, logp,batch=5000)
            cvae_df = pd.DataFrame({'SMILES': generated_smiles})
            cvae_df.to_csv(f'C:/Users/admin/Desktop/zhibao/zhibao_gen_vina{logp}.csv', index=False)
    print(f'Saving zhibao_gen_{logp}.csv ({cvae_df.shape[0]})...')
...
gen_smi = draw_molecules(cvae, logps=[-8,-9,-10])
